...
<form class="formBuscador" id="commentForm" name="buscador" action="?control=1&etapa=6&controlNoForm=1" method="post">
	<center>
...
<form class="formSolTit" id="form" name="solicitud_titulo" action="?etapa=6" method="post">
<?php


echo '<table align="center" cellspacing="1" cellpadding="4" border="1" bgcolor=#585858 id="tabla">';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>Listado de Alumnos - Ingreso Anal&iacute;tico</l1></td>';
	echo '</tr>';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>Fecha de Ingreso de Diploma:</l1>&nbsp;&nbsp;<input type="text" name="fechaIngreso" value="'.$fechaIngreso.'" id="date" placeholder="dd/mm/aaaa" onChange="submit()"/></td>';
	echo '</tr>';
	if($longVector!=0){
		$habilitarBoton = '<a href="confirmarSeleccion.php?etapa=6&longVector='.$longVector.'&textPasar='.$textPasar.'&fechaIngreso='.$fechaIngreso.'" ><input type="button" name="confirmar" value="Confirmar"/></a>';
	}else{
...
echo '<p>';
if($volver == 1){
	$redireccion = "ingresoAnalitico.php?control=0&controlNoForm=1&textPasar=$textPasar&longVector=$longVector";
}else{
	$redireccion = "";
}
