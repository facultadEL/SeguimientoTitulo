...
prevHtml += '<td id="titulo3" colspan="5" align="center"><l1>Listado de Alumnos - Ingreso Diploma</l1></td>';
prevHtml += '</tr>';
prevHtml += '<tr bgcolor="#FFFFFF">';
prevHtml += '<td id="titulo3" colspan="5" align="center"><l1>Fecha de Ingreso Diploma:</l1>&nbsp;&nbsp;<input type="text" name="fechaIngreso" value="'+fechaIngreso+'" id="date" placeholder="dd/mm/aaaa" onBlur="setFecha()" class="fallback" data-mask-clearifnotmatch="true" autoComplete="off"/></td>';
prevHtml += '</tr>';
prevHtml += '<tr bgcolor="#FFFFFF">';
...
function confirmSeleccion()
{
	...
	if((fechaIngreso != "") && (alumnosSeleccionados.length > 0))
	{
		...
		document.location.href = "confirmarSeleccion.php?alumnosPasar=" + alumnosPasar + "&etapa=" + etapa + "&fecha=" + fechaIngreso;
	}
	return false;
}
...
$consulta = "SELECT id_alumno,nro_expediente,apellido_alumno,nombre_alumno,nombre_carrera,numerodni_alumno,nombre_nivel_carrera,numerodni_alumno,foto_alumno,id_seguimiento,fecha_solicitud FROM alumno INNER JOIN seguimiento ON(seguimiento.alumno_fk = alumno.id_alumno) INNER JOIN carrera ON(carrera.id_carrera = seguimiento.carrera_fk) INNER JOIN nivel_carrera ON(carrera.nivel_carrera_fk = nivel_carrera.id_nivel_carrera) WHERE (fecha_rescs IS NOT NULL AND num_res_cs_fk IS NOT NULL) AND fecha_ingreso_diploma IS NULL ORDER BY id_nivel_carrera,id_carrera,apellido_alumno,nombre_alumno,id_alumno ASC";
