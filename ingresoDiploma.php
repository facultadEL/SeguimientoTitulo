...
<form class="formSolTit" id="form" name="solicitud_titulo" action="?etapa=5" method="post">
<?php
...
echo '<table align="center" cellspacing="1" cellpadding="4" border="1" bgcolor=#585858 id="tabla">';
	...
	if($longVector!=0 && $fechaIngreso != ''){
		$habilitarBoton = '<a href="confirmarSeleccion.php?etapa=5&longVector='.$longVector.'&textPasar='.$textPasar.'&fechaIngreso='.$fechaIngreso.'" ><input type="button" name="confirmar" value="Confirmar"/></a>';
	}else{
...
echo '<p>';
if($volver == 1){
	$redireccion = "ingresoDiploma.php?control=0&controlNoForm=1&textPasar=$textPasar&longVector=$longVector&fechaIngreso=$fechaIngreso";
}else{
	$redireccion = "";
}
