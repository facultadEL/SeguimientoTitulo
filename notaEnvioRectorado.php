<html>
<head>
<title> Nota Envio Rectorado</title>
...
prevHtml += '<td id="titulo3" colspan="7" align="center"><l1>Listado de Alumnos - Nota Envio Rectorado</l1></td>';
prevHtml += '</tr>';
prevHtml += '<tr bgcolor="#FFFFFF">';
prevHtml += '<td id="titulo3" colspan="7" align="center"><l1>N° de Nota:</l1>&nbsp;&nbsp;<input type="text" name="nroResolucion" value="'+nroResolucion+'" id="nroRes" onBlur="setNumero()" class="resolution" data-mask-clearifnotmatch="true" autoComplete="off"/></td>';
prevHtml += '</tr>';
prevHtml += '<tr bgcolor="#FFFFFF">';
prevHtml += '<td id="titulo3" colspan="7" align="center"><l1>Fecha de Nota:</l1>&nbsp;&nbsp;<input type="text" name="fechaIngreso" value="'+fechaIngreso+'" id="date" placeholder="dd/mm/aaaa" onKeyDown="ocultarMensajeFecha()" onBlur="controlFecha();setFecha()" class="fallback" data-mask-clearifnotmatch="true" autoComplete="off"/><br><span id="mensajeFecha" style="color:red;display:none">La fecha no puede ser mayor a la actual</span></td>';
prevHtml += '</tr>';
prevHtml += '<tr bgcolor="#FFFFFF">';
...
function mostrarAlumnos(busqueda)
{
	...
	if(busqueda == false)
	{
		...
		$.each(alumnosDiccionario, function(key,value)
		{
			...
			alumnosToAdd += '<tr><td align="center"><td align="center"><l2><font color="'+color+'">'+vStringAlumno[8]+'</font></l2></td><l2>'+vStringAlumno[1]+', '+vStringAlumno[2]+'</l2></td><td align="center"><l2>'+vStringAlumno[3]+'</l2></td><td align="center"><l2>'+vStringAlumno[4]+'</l2></td><td align="center"><l2>'+vStringAlumno[6]+'</l2></td><td align="center"><l2>'+vStringAlumno[7]+'</l2></td><td align="center"><input id="ctemario_general_curso" name="'+nombreCheck+'" type="checkbox" onChange="setAlumnoSelect('+key+')" '+checked+' /></td></tr>';
		});
	}
	else
	{
...
$consulta = "SELECT id_alumno,nro_expediente,apellido_alumno,nombre_alumno,nombre_carrera,numerodni_alumno,nombre_nivel_carrera,foto_alumno,id_seguimiento,fecha_solicitud,fecha_nota_envio_rec,archivo.nombre FROM alumno INNER JOIN seguimiento ON(seguimiento.alumno_fk = alumno.id_alumno) INNER JOIN carrera ON(carrera.id_carrera = seguimiento.carrera_fk) INNER JOIN nivel_carrera ON(carrera.nivel_carrera_fk = nivel_carrera.id_nivel_carrera) FULL OUTER JOIN archivo ON(seguimiento.num_nota_fk = archivo.id) WHERE (fecha_rescd IS NOT NULL AND num_res_cd_fk IS NOT NULL) AND (fecha_nota_envio_rec IS NULL OR num_nota_fk IS NULL) ORDER BY id_nivel_carrera,id_carrera,apellido_alumno,nombre_alumno,id_alumno ASC";
...
while($row = pg_fetch_array($val)){
	...
	$numeroNota = (empty($row['nombre'])) ? '' : $row['nombre'];

	$nroExpediente = (empty($row['nro_expediente'])) ? '' : $row['nro_expediente'];

	$stringAlumno = $row['id_seguimiento'].$sep.$row['apellido_alumno'].$sep.$row['nombre_alumno'].$sep.$row['nombre_carrera'].$sep.$row['nombre_nivel_carrera'].$sep.$row['numerodni_alumno'].$sep.$fechaNota.$sep.$numeroNota.$sep.$nroExpediente;
