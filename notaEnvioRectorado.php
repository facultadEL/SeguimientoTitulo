...
<form class="formBuscador" id="commentForm" name="buscador" action="?control=1&etapa=3&controlNoForm=1" method="post">
	<center>
		...
		<input type="hidden" name="nroNota" value="<?php echo $nroNota;?>" />
		<input type="hidden" name="fechaNota" value="<?php echo $fechaNota;?>" />
	<p>
		<input type="submit" name="buscar" value="Buscar"/>
	</p>
		</center>
</form>
<form class="formNotaRectorado" id="form" name="nota_rectorado" action="?etapa=3" method="post">
<?php


echo '<table align="center" cellspacing="1" cellpadding="4" border="1" bgcolor=#585858 id="tabla">';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>Listado de Alumnos - Nota Envio Rectorado</l1></td>';
	echo '</tr>';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>N&uacute;mero de nota:</l1>&nbsp;&nbsp;<input type="text" name="nroNota" value="'.$nroNota.'" id="nota" onChange="submit()" /></td>';
	echo '</tr>';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>Fecha de Nota:</l1>&nbsp;&nbsp;<input type="text" name="fechaNota" value="'.$fechaNota.'" id="date" placeholder="dd/mm/aaaa" onChange="submit()" /></td>';
	echo '</tr>';
	if($longVector!=0 && $nroNota != '' && $fechaNota != ''){
		$habilitarBoton = '<a href="confirmarSeleccion.php?etapa=3&longVector='.$longVector.'&textPasar='.$textPasar.'&nroNota='.$nroNota.'&fechaNota='.$fechaNota.'" ><input type="button" name="confirmar" value="Confirmar"/></a>';
	}else{
...
echo '<p>';
if($volver == 1){
	$redireccion = "notaEnvioRectorado.php?nroNota=".$nroNota."&fechaNota=$fechaNota&control=0&controlNoForm=1&textPasar=$textPasar&longVector=$longVector";
}else{
	$redireccion = "";
}
