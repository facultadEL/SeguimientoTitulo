...
<form class="formBuscador" id="commentForm" name="buscador" action="?control=1&etapa=2&controlNoForm=1" method="post">
	<center>
		...
		<input type="hidden" name="nroResolucion" value="<?php echo $nroResolucion;?>" />
		<input type="hidden" name="fechaResolucion" value="<?php echo $fechaResolucion;?>" />
	<p>
		<input type="submit" name="buscar" value="Buscar"/>
	</p>
		</center>
</form>
<form class="formResCd" id="form" name="resolucion_cd" action="?etapa=2" method="post">
<?php


echo '<table align="center" cellspacing="1" cellpadding="4" border="1" bgcolor=#585858 id="tabla">';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>Listado de Alumnos - Consejo Directivo</l1></td>';
	echo '</tr>';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>N&uacute;mero de resoluci&oacute;n:</l1>&nbsp;&nbsp;<input type="text" name="nroResolucion" id="resolucion" value="'.$nroResolucion.'" onChange="submit()"/></td>';
	echo '</tr>';
	echo '<tr bgcolor="#FFFFFF">';
		echo '<td id="titulo3" colspan="5" align="center"><l1>Fecha de resoluci&oacute;n:</l1>&nbsp;&nbsp;<input type="text" name="fechaResolucion" value="'.$fechaResolucion.'" id="date" placeholder="dd/mm/aaaa" onChange="submit()"/></td>';
	echo '</tr>';
	if($longVector!=0 && $nroResolucion != '' && $fechaResolucion != ''){
		$habilitarBoton = '<a href="confirmarSeleccion.php?etapa=2&longVector='.$longVector.'&textPasar='.$textPasar.'&nroResolucion='.$nroResolucion.'&fechaResolucion='.$fechaResolucion.'" ><input type="button" name="confirmar" value="Confirmar"/></a>';
	}else{
...
echo '<p>';
if($volver == 1){
	$redireccion = "resolucionCd.php?nroResolucion=".$nroResolucion."&control=0&controlNoForm=1&textPasar=$textPasar&longVector=$longVector&fechaResolucion=$fechaResolucion";
}else{
	$redireccion = "";
}
echo '<center>'.$habilitarBoton.'&nbsp;&nbsp;<a href="'.$redireccion.'"><input type="button" value="Atr&aacute;s"></a></center>';
echo '<p>';
?>
</form>
</body>
